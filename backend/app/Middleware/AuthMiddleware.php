...
final class AuthMiddleware
{
    public function __invoke(Request $request): void {
        ...
        $hasMustChange=$this->hasPasswordMustChangeColumn();
        $sql='SELECT u.id,u.fullname,u.email,u.phone,u.address,u.barangay,u.role,u.municipality_id,u.status,m.municipality_name'.($hasMustChange?',u.password_must_change':'').'
    ...
    private function hasPasswordMustChangeColumn(): bool
    {
        $s=Database::connection()->prepare('SHOW COLUMNS FROM users LIKE "password_must_change"');$s->execute();return (bool)$s->fetch();
    }
}
