...
{
    private const TYPES=['flood_zone','storm_surge_area','earthquake_area','high_risk_area'];
    ...
    {
        $d=$r->body();$required=['hazard_name','hazard_type','risk_level','geojson_data'];if($r->user['role']==='admin')$required[]='municipality_id';Validator::require($d,$required);$errors=[];$name=trim((string)$d['hazard_name']);$description=trim((string)($d['description']??''));$mid=$r->user['role']==='subadmin'?(int)$r->user['municipality_id']:(int)$d['municipality_id'];
        if(strlen($name)<2||strlen($name)>180)$errors['hazard_name']='Must contain 2 to 180 characters';if(!in_array($d['hazard_type'],self::TYPES,true))$errors['hazard_type']='Invalid hazard type';if(!in_array($d['risk_level'],self::RISKS,true))$errors['risk_level']='Invalid risk level';if(strlen($description)>5000)$errors['description']='Must not exceed 5000 characters';
        ...
        return ['hazard_name'=>$name,'hazard_type'=>$d['hazard_type'],'municipality_id'=>$mid,'risk_level'=>$d['risk_level'],'description'=>$description?:null,'geojson_data'=>$encoded];
    }
