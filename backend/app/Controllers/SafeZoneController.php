...
final class SafeZoneController
{
    ...
    {
        $d=$r->body();$required=['safezone_name','address','latitude','longitude','capacity'];if($r->user['role']==='admin')$required[]='municipality_id';Validator::require($d,$required);$errors=[];$name=trim((string)$d['safezone_name']);$address=trim((string)$d['address']);$contactPerson=trim((string)($d['contact_person']??''));$contactNumber=trim((string)($d['contact_number']??''));$description=trim((string)($d['description']??''));$lat=filter_var($d['latitude'],FILTER_VALIDATE_FLOAT);$lng=filter_var($d['longitude'],FILTER_VALIDATE_FLOAT);$capacity=filter_var($d['capacity'],FILTER_VALIDATE_INT,['options'=>['min_range'=>0]]);$mid=$r->user['role']==='subadmin'?(int)$r->user['municipality_id']:(int)$d['municipality_id'];
        if(strlen($name)<2||strlen($name)>180)$errors['safezone_name']='Must contain 2 to 180 characters';if(strlen($address)>255)$errors['address']='Must not exceed 255 characters';if(strlen($contactPerson)>120)$errors['contact_person']='Must not exceed 120 characters';if(strlen($contactNumber)>30)$errors['contact_number']='Must not exceed 30 characters';if(strlen($description)>5000)$errors['description']='Must not exceed 5000 characters';if($lat===false||abs((float)$lat)>90)$errors['latitude']='Must be between -90 and 90';if($lng===false||abs((float)$lng)>180)$errors['longitude']='Must be between -180 and 180';if($capacity===false)$errors['capacity']='Must be a non-negative whole number';
        ...
        return ['safezone_name'=>$name,'municipality_id'=>$mid,'address'=>$address,'latitude'=>(float)$lat,'longitude'=>(float)$lng,'capacity'=>(int)$capacity,'contact_person'=>$contactPerson?:null,'contact_number'=>$contactNumber?:null,'description'=>$description?:null];
    }
