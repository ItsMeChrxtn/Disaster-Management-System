...
final class Hazard extends BaseModel
{
    ...
    private const STANDARD_TYPES=['flood_zone','storm_surge_area','landslide','earthquake_area','fire'];
    public function all(array $filters): array
    {
        $where=['h.status="active"'];$values=[];
        if($filters['hazard_type']!==null){
            if($filters['hazard_type']==='other'){
                $where[]='h.hazard_type NOT IN ('.implode(',',array_fill(0,count(self::STANDARD_TYPES),'?')).')';
                array_push($values,...self::STANDARD_TYPES);
            }else{$where[]='h.hazard_type=?';$values[]=$filters['hazard_type'];}
        }
        foreach(['risk_level','municipality_id'] as $field)if($filters[$field]!==null){$where[]='h.'.$field.'=?';$values[]=$filters[$field];}
