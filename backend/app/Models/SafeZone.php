...
{
    private const SELECT='SELECT s.id,s.safezone_name,s.municipality_id,s.address,s.latitude,s.longitude,s.capacity,s.contact_person,s.contact_number,s.description,s.status,s.created_at,s.updated_at,m.municipality_name FROM safe_zones s JOIN municipalities m ON m.id=s.municipality_id';
    ...
    {
        $s=$this->db->prepare('INSERT INTO safe_zones (safezone_name,municipality_id,address,latitude,longitude,capacity,contact_person,contact_number,description,status) VALUES (?,?,?,?,?,?,?,?,?,"active")');$s->execute([$d['safezone_name'],$d['municipality_id'],$d['address'],$d['latitude'],$d['longitude'],$d['capacity'],$d['contact_person'],$d['contact_number'],$d['description']]);return (int)$this->db->lastInsertId();
    }
    public function update(int $id,array $d): void { $this->db->prepare('UPDATE safe_zones SET safezone_name=?,municipality_id=?,address=?,latitude=?,longitude=?,capacity=?,contact_person=?,contact_number=?,description=? WHERE id=?')->execute([$d['safezone_name'],$d['municipality_id'],$d['address'],$d['latitude'],$d['longitude'],$d['capacity'],$d['contact_person'],$d['contact_number'],$d['description'],$id]); }
